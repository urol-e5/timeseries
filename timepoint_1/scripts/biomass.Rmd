...
sh <- tibble(afdw) %>%
  ...
  mutate(SH = Sym / Host) %>%
  filter(SH < 0.5)   # removes ones outlier sample POC-375 that had S/H = 0.7.... somethings wrong with that sample.
...
afdw %>%
  ...
  select(colony_id, Pan.ID, partner, initial.mass.g, dry.pan.mass.g, burnt.pan.mass.g, dry.biomass.g, burnt.biomass.g,
         homog_vol_ml, surface.area.cm2, DW.mg.cm2, AFDW.mg.cm2)
...
```{r}
afdw %>%
  select(colony_id, partner, site, species, DW.mg.cm2, AFDW.mg.cm2) %>%
