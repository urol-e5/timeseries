...
chl.summ <- chl %>%
  select(colony_id, species, site, chla.ug.cm2, chlc2.ug.cm2) %>%
  gather(metric, value, -colony_id, -species, -site) %>%
  group_by(species, site, metric) %>%
  summarise(mean.value = mean(value, na.rm = TRUE),
            std.error = plotrix::std.error(value, na.rm = TRUE))

# Plot
ggplot(chl.summ, aes(x = site, y = mean.value, shape = metric, color = species)) +
  facet_grid(~species) +
  geom_point(size = 3, position = position_dodge(width = 0.5)) +
  geom_errorbar(width = 0.5, 
                aes(ymin = mean.value - std.error, ymax = mean.value + std.error),
