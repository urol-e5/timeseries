...
pH <- read.csv("data/20200102_SN20697597_Site1_pH.csv", header=F, sep=",", na.strings="NA", skip=700) #load  data
pH <- pH[,1:4]
colnames(pH) <- c("Date.Time", "Temp", "mV", "pH")
pH$Date.Time <- parse_date_time(pH$Date.Time, "Y!-m!*-d! H!:M!:S!") 

pdf("output/20200106_pH_Temp.pdf")
par(mfrow=c(3,1))
par(mar = c(5,5,2,5))
...
pH <- read.csv("data/20200103_SN20697596_Site2_pH.csv", header=F, sep=",", na.strings="NA", skip=700) #load  data
pH <- pH[,1:4]
colnames(pH) <- c("Date.Time", "Temp", "mV", "pH")
pH$Date.Time <- parse_date_time(pH$Date.Time, "Y!-m!*-d! H!:M!:S!") 
...
pH <- read.csv("data/20200106_20697595_Site3_pH.csv", header=F, sep=",", na.strings="NA", skip=700) #load  data
pH <- pH[,1:4]
colnames(pH) <- c("Date.Time", "Temp", "mV", "pH")
pH$Date.Time <- parse_date_time(pH$Date.Time, "Y!-m!*-d! H!:M!:S!") 
