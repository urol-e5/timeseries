...
data$colony_id <- gsub("POR-261", "POR-216", data$colony_id)
data$colony_id <- gsub("POR-387", "POR-385", data$colony_id)

data <- full_join(data, SA, by = "colony_id")

## read in corrected salinity values 
corr.salinity <- read.csv("data/1_calcification/corrected_salinity.csv", header=TRUE) %>%
...
# bk-2 spilled out during volume measurement - replaced with another chamber 1 used as BK value
data$Vol.L[which(data$colony_id == "BK-2")] <- 0.610
```
...
#Note here I am summarizing a mean for every colony to remove the duplicate POC-50. Come back to this to see why we have a duplicate.  
...
NEC %>%
  ...
  summarise(umol.cm2.hr=mean(umol.cm2.hr))%>%
  write_csv(path = "output/1_calcification_output.csv")
```
