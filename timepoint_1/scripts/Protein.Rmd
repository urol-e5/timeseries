...
std_curve_plot + geom_line(data = fitted, aes(x = abs562.adj, y = .fitted))
```
...
prot <- df %>%
  ...
  mutate(abs562.adj = abs562 - std_curve$abs562[std_curve$std == "I"],
         prot_ug.mL = map_dbl(abs562.adj, ~ predict(mod, newdata = data.frame(abs562.adj = .))))

std_curve_plot + 
  geom_point(data = prot, aes(x = abs562.adj, y = prot_ug.mL), pch = "X", cex = 3, alpha = 0.3) 
```
