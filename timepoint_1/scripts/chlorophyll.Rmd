...
For 9 colonies we need to take the values from Plate 2 and remove the ones from Plate 1. And use Plate 2 for the ones that are duplicates! Need to code this in this script.  Need to track down each colony 

Calculate the variance between two replicates and then show those that are above a cutoff that isn't acceptable - then use those that are acceptable.  Flag anything >0.1 or some cutoff that we deem acceptable.  

QC for "Are there duplicates in final output file"




# ```{r}
# chl_data <- read_csv("data/1_chlorophyll/1_20200106_Chl", skip = 24, n_max = 24) %>%
#     select(-1) %>%
