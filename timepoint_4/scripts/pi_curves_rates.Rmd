...
## Import data
```{r}
path.p <- "data/4_pi_curves/" #the location of all your respirometry files 
...
sample.info <- read_csv(file = "data/4_pi_curves/4_pi_curves_sample_metadata.csv")

# Load PI curve run metadata (i.e., light levels and interval times for each run)
run.info <- read_csv(file = "data/4_pi_curves/4_pi_curves_run_metadata.csv")
...
##ERROR HERE: Problem with `mutate()` input `regs`. x invalid 'length.out' argument
df <- df %>%
  filter(!colony_id==c("ACR-220", "ACR-225", "ACR-229", "ACR-343", "ACR-368", "BK-3", "BK-5", "POC-239", "POC-248", "POC-254", "POC-358", "POC-359", "POC-366", "POC-369", "POR-221", "POR-242", "POR-245", "POR-338", "POR-340"))%>%
  dplyr::mutate(regs = furrr::future_map(thin_data, function(.) {       # future_map executes function in parallel
    group_by(., Light_Value) %>%
    do(rankLcRg = fit_reg(.))
  }))

## Tried manually subsetting colonies to see which colony is causing the problem. 

#colonies that generate the issue individually: 
#ACR-220 
#ACR-225 
#ACR-229 
#ACR-343
...
test <- df %>%
       filter(colony_id==c("POR-385"))%>%
        dplyr::mutate(regs = furrr::future_map(thin_data, function(.) {       # future_map executes function in parallel
       group_by(., Light_Value) %>%
        do(rankLocRg = fit_reg(.))
  }))


#check if this is due to inability to calculate slope due to low sample size or variation at certain light levels in these colonies

#check file formats for column issues
...
# Write to output file
write.csv(pr.out, "output/4_pi_curve_rates.csv")
```
