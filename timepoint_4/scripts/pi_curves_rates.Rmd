...
## Tried manually subsetting colonies to see which colony is causing the problem. In this script (filtering to the first 5 colonies) only 3 of the colonies are running in this data set??? I can't figure out why... 
test <- df %>%
       filter(colony_id==c("ACR-140", "ACR-145", "ACR-150", "ACR-173", "ACR-178"))%>%
        dplyr::mutate(regs = furrr::future_map(thin_data, function(.) {       # future_map executes function in parallel
       group_by(., Light_Value) %>%
        do(rankLocRg = fit_reg(.))
  }))

levels(as.factor(test$colony_id))
