...
homog_vols <- read_csv("data/3_3homogenate_vols/3_3homogenate_vols.csv") %>% 
  pull(colony_id)

surface_area <- read_csv("data/3_3surface_area/3_3surface_area_data.csv") %>% 
  filter(!grepl("Standard", Sample)) %>%
  pull(colony_id)

chl <- list.files("data/3_3chlorophyll/", "platemap", full.names = TRUE) %>%
  map_df(read_csv) %>%
  drop_na() %>%
  filter(!colony_id == "BK") %>%
  pull(colony_id)

afdw <- read_csv("data/3_3biomass/3_3biomass_data.csv") %>%
  drop_na() %>%
  pull(colony_id)

molec <- read_csv("data/3_3molecular_backups/3_3molecular_backups.csv") %>%
  distinct(colony_id) %>%
  pull()

picurve <- list.files("data/3_3pi_curves/", "*.csv") %>%
  gsub("_.*.csv", "", .) %>%
  subset(!grepl("BK", .))

sym_counts <- read_csv("data/3_3sym_counts/3_3sym_counts_data.csv") %>%
  pull(colony_id)

#tac

tac <- list.files("data/3_3antioxidant_capacity/", "platemap", full.names = TRUE) %>%
  map_df(read_csv) %>%
  drop_na() %>%
  filter(!colony_id == "BK") %>%
  pull(colony_id)
...
protein <- list.files("data/3_3protein/", "platemap", full.names = TRUE) %>%
  map_df(read_csv) %>%
  drop_na() %>%
  filter(!colony_id == "BK") %>%
  pull(colony_id)
