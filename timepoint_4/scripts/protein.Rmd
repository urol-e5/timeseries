...
prot_path = "data/4_protein/"                                              # Path to prot data directory
...
df <- tibble(file = prot_data_files) %>%
  ...
  dplyr::mutate(platemap = map(plate, ~read_csv(paste0(prot_path, ., "_BCA_platemap.csv"))),
...
prot <- df %>%
  ...
  mutate(colony_id = factor(ifelse(colony_id == "ACR-231", "ACR-237", colony_id)))%>%
  group_by(colony_id) %>%          
...
prot %>%
  ...
  write_csv(., path = "output/4_protein.csv")
```
