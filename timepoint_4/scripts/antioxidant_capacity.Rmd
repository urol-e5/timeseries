...
# List TAC data files
tac_path = "data/4_antioxidant_capacity/"                                                 # Path to TAC data directory
...
std_curve <- df %>%
  ...
  filter(plate=="4_20201118_Plate3")%>%
  left_join(standards)

#remove well B8 as an outlier - initial value far out of range of other initial values for wells 
std_curve<-std_curve[!(std_curve$well=="B8"),]
...
homog_vols <- read_csv("data/4_homogenate_vols/4_homogenate_vols.csv") %>% select(1:2)

# Protein data
prot <- read_csv("output/4_protein.csv")
...
tac %>%
  ...
  write_csv(., path = "output/4_antioxidant_capacity.csv")
```
