...
# Import data
```{r import_data}
Data <- read.csv(file = 'output/4_pi_curve_rates.csv')
#Data <- Data[1:40,]
```
...
#nls_data <- Data %>% 
   ...
   #mutate(model1 = map(data, ~ 
                        #nls(Pc ~ (Am*((AQY*PAR)/(sqrt(Am^2 + (AQY*PAR)^2)))-Rd), data=., start=list(Am=1,  AQY=0.002, Rd=.2)) %>%
                           # tidy %>%
...
# mutate(timepoint="timepoint4")%>%
  #write_csv(., "output/4_pi_curve_pars_NLS.csv")
```
...
augmented <- Data %>% 
  nest(-colony_id) %>% 
  mutate(
    fit = map(data, ~ nls(Pc ~ (Am*((AQY*PAR)/(sqrt(Am^2 + (AQY*PAR)^2)))-Rd), data=., start=list(Am=(max(.$Pc)-min(.$Pc)),  AQY=0.005, Rd=-min(.$Pc)))),
