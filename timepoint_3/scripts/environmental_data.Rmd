...
Assemble full dataset of environmental data for timepoint 3  
```{r}
#pH & temp
head(pH1)
head(pH2)
...
pH_temp_out<-rbind(pH1,pH2)%>%
  ...
  mutate(timepoint=c("timepoint3"))%>%
  mutate(Date.Time=as.character(Date.Time))%>%
  write_csv(., "output/timepoint3_pH_temp.csv")
...
light_out<-rbind(S1.Data, S2.Data)%>%
  ...
  mutate(timepoint=c("timepoint3"))%>%
  mutate(Date.Time=as.character(Date.Time))%>%
  write_csv(., "output/timepoint3_light.csv")
...
hobo_out<-rbind(Temp1, Temp2)%>%
  ...
  mutate(timepoint=c("timepoint3"))%>%
  mutate(Date.Time=as.character(Date.Time))%>%
  write_csv(., "output/timepoint3_hobo.csv")
```