...
chl_path <- "data/3_chlorophyll/"                                        # Path to chlorophyll data directory
...
homog.vol <- read_csv("data/3_homogenate_vols/3_homogenate_vols.csv") %>%
  select(colony_id, homog_vol_ml)
chl <- full_join(df, homog.vol)

# Load surface area
sa <- read_csv("output/3_surface_area.csv")
