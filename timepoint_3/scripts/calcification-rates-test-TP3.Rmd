...
data %>% filter(!is.na(Ta.norm)) %>% mutate(titration.location = case_when(
    endsWith(titration.run, "CSUN.csv") ~ "CSUN",
    endsWith(titration.run, "Lab.csv") ~ "URI")) %>%
  ggplot(aes(x=sample.type, y=Ta.norm, color = Run.Number))  + theme_bw() + geom_point() + facet_grid(~titration.location)
```
...
NEC %>% filter(is.na(surface.area.cm2)) # missing SA for these corals 
