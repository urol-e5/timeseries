...
prot_path = "data/3_protein/"                                              # Path to prot data directory
...
# Fit linear model
lmod <- lm(BSA_ug.mL ~ abs562.adj, data = std_curve)
fitted <- lmod %>% broom::augment()
...
prot <- df %>%
  ...
  mutate(abs562.adj = abs562 - std_curve$abs562[std_curve$std == "I"],     # Subtract blank standard (Standard I) from mean absorbance values
         prot_ug.mL = map_dbl(abs562.adj, ~ predict(lmod, newdata = data.frame(abs562.adj = .))))    # Use standard curve to convert absorbance to protein
...
homog_vols <- read_csv("data/3_homogenate_vols/3_homogenate_vols.csv") %>% select(1:2)
...
prot %>%
  ...
  write_csv(., path = "output/3_protein.csv")
```
