...
## Import data
```{r}
path.p <- "data/3_pi_curves/" #the location of all your respirometry files 
...
sample.info <- read_csv(file = "data/3_pi_curves/3_pi_curves_sample_metadata.csv")

# Load PI curve run metadata (i.e., light levels and interval times for each run)
run.info <- read_csv(file = "data/3_pi_curves/3_pi_curves_run_metadata.csv")
...
## THIS CODE IS GIVING ERROR "Error: Problem with `mutate()` input `intervals`.x invalid number of intervals Input `intervals` is `map2(...)`.
df <- df %>%
  ...
  mutate(data = map(intervals, ~ unnest(tibble(.), .id = "Light_Value"))) ### NOT WORKING 
...
# #troubleshooting code issue
# data0 <- df[[4]][[1]] #check data
# info <- df[[3]][[1]] #check info
# 
# #check number of intervals
# intervals <- split(data0, f = cut(as.numeric(data0$Time), breaks = as.numeric(c(info$Start.time, last(info$Stop.time)))))
# 
# #check labels
# as.character(info$Light_Value)
...
# Write to output file
write.csv(pr.out, "output/3_pi_curve_rates.csv")
```
