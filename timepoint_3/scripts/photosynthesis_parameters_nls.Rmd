...
PI Curve Step #3: Explore photosynthesis data from nls models  
...
```{r}
pars <- read_csv("output/3_pi_curve_pars_nls.csv")
```
...
curves %>% 
  ...
  labs(x = expression(paste('PAR (', mu, "mol photons m"^-2, 's'^-1,")")),
       ...
       title = "Timepoint 3: September, 2020")
dev.off()

# Save mean photosynthesis pars for each species/site to file
meanpars %>%
  mutate(timepoint="timepoint3") %>%
  write_csv(path = "output/3_photosynthesis_pars_summary_nls.csv")
```
