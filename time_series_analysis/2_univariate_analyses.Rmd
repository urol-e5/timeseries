...
aqy_plot<-master %>%
  ...
  ggplot(., aes(x = site_code, y = AQY, fill = timepoint, group=interaction(site_code,timepoint))) +
    ...
    facet_wrap(~species) +
    xlab("Site") + 
    ylab(expression(bold(paste("Quantum Yield (α)"))))+
    theme_classic() + 
    theme(
      legend.position="none",
      legend.title=element_text(face="bold", size=14),
...
rd_plot<-master %>%
  ...
  ggplot(., aes(x = site_code, y = Rd, fill = timepoint, group=interaction(site_code,timepoint))) +
    ...
    facet_wrap(~species) +
    xlab("Site") + 
    ylab(expression(bold(paste("Respiration (µmol O" [2], "cm"^-2, " h"^-1, ")"))))+
    theme_classic() + 
    theme(
      legend.title=element_text(face="bold", size=14),
      legend.text=element_text(size=14),
...
The most highly correlated variables are: 
- Max photosynthesis and respiration 
- Host and symbiont biomass
- Host biomass and max photosynthesis 
- Chlorophyll a and chlorophyll c2
- Antioxidant capacity and symbiont biomass 
- Fruther correlations between host/symbiont biomass and metabolic rates 
