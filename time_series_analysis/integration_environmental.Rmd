...
radar<-radar_data%>%
  select(site, pH_mean, FullTemp_mean, light_mean, FullTemp_sd, pH_sd, FullTemp_max, FullTemp_min, pH_max, pH_min, FullTemp_range, pH_range, light_max)%>%
...
png(filename="Figures/Environmental_Radar.png", width=2280, heigh=1880, units="px", res=300)
...
```{r}
summary(aov(FullTemp_mean~site, radar_data))
summary(aov(FullTemp_sd~site, radar_data))
summary(aov(FullTemp_max~site, radar_data))
summary(aov(FullTemp_min~site, radar_data))
summary(aov(FullTemp_range~site, radar_data))
```

Temperature daily variability (stdev), max daily temperature, min daily temperature, and daily temperature range are different by site.  
...
```{r}
length(radar$FullTemp_mean)/3
```

We have 68 days of data from all variables.  
...
Make new boxplots faceted by time point. 
...
ggsave(filename="Figures/temp_boxplots.jpg", plot=temp_fig, dpi=200, width=24, height=6, units="in")
...
ggsave(filename="Figures/pH_boxplots.jpg", plot=pH_fig, dpi=200, width=24, height=6, units="in")
```
...
ggsave(filename="Figures/light_boxplots.jpg", plot=light_fig, dpi=200, width=16, height=6, units="in")
```
