...
To do this, we need to first summarize data by a unit of time becuase the loggers differ by seconds/minutes, producing a lot of na's. We can first summarize by day for temperature (using Hobo here), pH, and light. 

Calculate additional metrics of daily min/max, range, variability, and number of days above 30C.  

```{r}
radar_data<-master_qc %>%
    mutate(Date=as.factor(as.Date(Date.Time)))%>%
    select(Date, site, Temp, pH, light)%>%
    group_by(Date, site)%>%
    summarise(across(Temp:light, list(mean=mean, min=min, max=max, sd=sd), na.rm=TRUE))%>%
    mutate(Temp_range=(Temp_max-Temp_min), na.rm=TRUE)%>%
    mutate(pH_range=(pH_max-pH_min), na.rm=TRUE)

radar_data[mapply(is.infinite, radar_data)] <- NA
radar_data[mapply(is.nan, radar_data)] <- NA

#radar plot 

radar<-radar_data%>%
  select(site, pH_mean, Temp_mean, light_mean, Temp_sd, pH_sd, Temp_max, Temp_min, pH_max, pH_min, Temp_range, pH_range, light_max)%>%
...
png(filename="Figures/Environmental_Radar.png", width=1980, heigh=1880, units="px", res=300)
ggRadar(radar, aes(colour=site), scales="fixed")+ggplot2::theme_minimal()+ggplot2::theme(text=element_text(color="black"))
dev.off()
...
```{r}
summary(aov(Temp_mean~site, radar_data))
summary(aov(Temp_sd~site, radar_data))
summary(aov(Temp_max~site, radar_data))
summary(aov(Temp_min~site, radar_data))
summary(aov(Temp_range~site, radar_data))
```

Temperature daily variability (stdev), max daily temperature, and daily temperature range are different by site.  

*Light variables*
```{r}
summary(aov(light_mean~site, radar_data))
summary(aov(light_max~site, radar_data))
```
...
*pH variables*  
```{r}
summary(aov(pH_mean~site, radar_data))
summary(aov(pH_sd~site, radar_data))
summary(aov(pH_max~site, radar_data))
summary(aov(pH_min~site, radar_data))
summary(aov(pH_range~site, radar_data))
```

Mean daily pH, daily pH variability (stdev), max daily pH, min daily pH, and pH daily range are different by site.  

Number of days of data included in this data set:  
```{r}
length(radar$Temp_mean)/3
```

We have 204 days of data from all variables.  
