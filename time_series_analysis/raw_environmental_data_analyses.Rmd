...
# Output temperature data for MOSAIC 

```{r}
head(master_qc)
dim(master_qc)

temp_molecular<-master_qc%>%
  mutate(site=if_else(site=="site2", "Mahana", 
                      if_else(site=="site3", "Hilton", 
                              if_else(site=="site1", "Manava", NA))))%>%
  select(Date.Time, site, FullTemp)%>%
  rename(date.time=Date.Time, temp.C=FullTemp)%>%
  filter(!is.na(temp.C))

dim(temp_molecular)

write_csv(temp_molecular, "Output/temperature_timeseries.csv")
...
temp_summary_plot <- master_qc %>%
  ...
  scale_colour_manual(name="Site", values=c("#374d7c", "#00cccc", "#ff6633"), labels=c("Orovau", "Vaipahu", "Matotia")) +
  ylab("Temperature (°C)") + 
  xlab("Date") +
...
## Maximum monthly mean calculations 

Calculate maximum monthly mean from our time series data.  

```{r}
## Calcualte MMM from our dataset
mmm<-master_qc %>%
    ...
    group_by(month) %>%
    summarize(monthly_mean = mean(FullTemp, na.rm = TRUE), .groups = "drop_last") %>%
    summarize(MMM = max(monthly_mean, na.rm = TRUE)) %>%
    pull(MMM);mmm
```

## Overlay MMM onto temperature plot 

```{r}
temp_summary_plot_MMM <- master_qc %>%
  ggplot(., aes(x=Date.Time, y=FullTemp)) + 
  geom_point(colour="gray", size=0.5, alpha=0.05) + 
  geom_smooth(aes(colour=site), se=FALSE, method="gam") + 
  scale_colour_manual(name="Site", values=c("#374d7c", "#00cccc", "#ff6633"), labels=c("Orovau", "Vaipahu", "Matotia")) +
  ...
  geom_vline(xintercept = as.POSIXct("2020-09-08"), linetype="dashed")+
  geom_vline(xintercept = as.POSIXct("2020-11-01"), linetype="dashed")+
  geom_hline(yintercept=mmm, linetype="dotted")+
  theme_classic() + 
  theme(
    text=element_text(size=12, colour="black"), 
    axis.title=element_text(size=14, colour="black", face="bold"), 
    axis.text.x=element_text(size=10, colour="black", angle=45, hjust=1, vjust=1),
    legend.title=element_text(size=14, colour="black", face="bold")
  );temp_summary_plot_MMM

ggsave(filename="Figures/Environmental/Environmental_Temperature_summary_MMM.png", temp_summary_plot_MMM, width = 9, height = 4, units = c("in"))
ggsave(filename="Figures/Environmental/Environmental_Temperature_summary_MMM.pdf", temp_summary_plot_MMM, width = 9, height = 4, units = c("in"))
```
