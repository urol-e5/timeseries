...
permanova_matrix_acr <- pmdf_heatmap %>%
  ...
  filter(!timepoint=="TP2a")%>%
  filter(!timepoint=="TP2b")%>%
   filter(!colony_id %in% c("ACR-178", "ACR-210", "ACR-213", "ACR-218", "ACR-220", "ACR-241", "ACR-247", "ACR-267", "ACR-347", "ACR-360"))%>% #remove colonies not present in phys dataset or with single timepoint samples 
  droplevels()
...
#                 Df SumOfSqs      R2      F Pr(>F)
# site             2   0.1860 0.00791 0.4396  0.770
# timepoint        3   0.2992 0.01273 0.4715  0.853
...
output_matrix <- pmdf_heatmap %>%
  ...
  filter(!timepoint=="TP2b")%>%
  mutate(sample_id=paste0(colony_id_corr, "_", timepoint))%>%
  ...
  select(!ID)%>%
  select(sample_id, colony_id, colony_id_corr, timepoint, site, nutrient, site_code, month, haplotype, everything())
...
plot_species_df<-output_matrix%>%
  group_by(species)%>%
   filter(!colony_id_corr %in% c("ACR-178", "ACR-210", "ACR-213", "ACR-218", "ACR-220", "ACR-241", "ACR-247", "ACR-267", "ACR-347", "ACR-360"))%>% #remove colonies not present in phys dataset or with single timepoint samples 
...
get_custom_color_poc <- function(Profile) {
  if (startsWith(Profile, "C1/C42.2")) {
    ...
    return("orange")
  } else if (startsWith(Profile, "C3/C115d")) {
    return("orange2")
  } else if (startsWith(Profile, "C1d/C42.2")) {
    return("darkgoldenrod3")
  } else if (startsWith(Profile, "C1d")) {
    return("darkgoldenrod2")
  } else if (startsWith(Profile, "C15/C93a")) {
    return("chocolate3")
  } else if (startsWith(Profile, "C15h/C3")) {
    return("chocolate4")
    ...
    return("yellow2")
  } else if (startsWith(Profile, "D1-D17d")) {
    return("red3")
  } else if (startsWith(Profile, "D1-D1u")) {
    return("red4")
  } else if (startsWith(Profile, "D1/D4")) {
    return("red2")
  } else {
...
get_custom_color_por <- function(Profile) {
  if (startsWith(Profile, "C116")) {
    ...
    return("darkred")
  } else if (startsWith(Profile, "A1/A1ee")) {
    return("darkgray")
  } else if (startsWith(Profile, "C91")) {
    return("orange2")
  } else if (startsWith(Profile, "D1")) {
...
scaled_acr<-prcomp(pca_df_acr[c(5:8)], scale=TRUE, center=TRUE) 
```

```{r}
# scale data
vegan_acr <- scale(pca_df_acr[ ,5:8])
...
scaled_all<-prcomp(pca_df_all[c(5:39)], scale=TRUE, center=TRUE) 
```

```{r}
# scale data
vegan_all <- scale(pca_df_all[ ,5:39])
...
# Generate sample profile plots for each species/haplotype 

Acropora
```{r}
ind_plot1<- output_matrix%>%
  filter(species=="Acropora")%>%
 filter(!colony_id_corr %in% c("ACR-178", "ACR-210", "ACR-213", "ACR-218", "ACR-220", "ACR-241", "ACR-247", "ACR-267", "ACR-347", "ACR-360"))%>% #remove colonies not present in phys dataset or with single timepoint samples 
  group_by(colony_id, species, haplotype)%>%
  summarise_if(is.numeric, mean, na.rm = TRUE, .groups = "drop")%>%
  select_if(~ !is.numeric(.) || sum(.) >= 0.01) %>% #remove taxa that are below 1% relative abundance
  droplevels()%>%
  pivot_longer(-c(colony_id, species,haplotype), values_to="Abundance", names_to="Profile")%>%
  
  ggplot(aes(x = colony_id, y = Abundance)) + 
  facet_grid(~haplotype)+
  geom_bar(position="stack", stat="identity", aes(fill = Profile), color = "black") +
  scale_fill_manual(values = sapply(plot_species_df$Profile, get_custom_color_acr))+
  theme_classic() +
  ...
  #facet_nested(~ species, scales = "free_x", space = "free_x") +
  labs(x="", fill="ITS2 Profile", y="Relative Abundance");ind_plot1
```

Pocillopora
```{r}
ind_plot2<- output_matrix%>%
  filter(species=="Pocillopora")%>%
  group_by(colony_id, species, haplotype)%>%
  summarise_if(is.numeric, mean, na.rm = TRUE, .groups = "drop")%>%
  select_if(~ !is.numeric(.) || sum(.) >= 0.01) %>% #remove taxa that are below 1% relative abundance
  ...
  ggplot(aes(x = colony_id, y = Abundance)) + 
  facet_grid(~haplotype, scales = "free_x", space="free_x")+
  geom_bar(position="stack", stat="identity", aes(fill = Profile), color = "black") +
  scale_fill_manual(values = sapply(plot_species_df$Profile, get_custom_color_poc))+
  theme_classic() +
  theme(axis.text.y = element_text(colour = 'black', size = 10), 
  ...
  #facet_nested(~ species, scales = "free_x", space = "free_x") +
  labs(x="", fill="ITS2 Profile", y="Relative Abundance");ind_plot2
```

Porites 
```{r}
ind_plot3<- output_matrix%>%
  filter(species=="Porites")%>%
  group_by(colony_id, species, haplotype)%>%
  summarise_if(is.numeric, mean, na.rm = TRUE, .groups = "drop")%>%
  select_if(~ !is.numeric(.) || sum(.) >= 0.01) %>% #remove taxa that are below 1% relative abundance
  ...
  ggtitle(expression("E"))+
  
  #facet_nested(~ species, scales = "free_x", space = "free_x") +
  labs(x="", fill="ITS2 Profile", y="Relative Abundance");ind_plot3
```

# Plot communities by haplotype by relative abundance by site  
...
plot_species_site_df_poc<-output_matrix%>%
  ...
  group_by(colony_id, site, species, haplotype)%>%
  summarise_if(is.numeric, mean, na.rm = TRUE, .groups = "drop")%>%
  select_if(~ !is.numeric(.) || sum(.) >= 0.01) %>% #remove taxa that are below 1% relative abundance
  group_by(species, site, haplotype)%>%
  summarise_if(is.numeric, mean, na.rm = TRUE, .groups = "drop")%>%
...
# Plot summarized by haplotype summarized across sites 
...
plot_species_site_acr<-ggplot(plot_species_site_df_acr, aes(x = haplotype, y = Abundance)) + 
  ...
  #facet_nested(~ species, scales = "free_x", space = "free_x") +
  labs(x="", fill="ITS2 Profile", y="Relative Abundance")+
  theme(text=element_text(size=20), 
        strip.text=element_text(size=20), 
        axis.text=element_text(size=20), 
        axis.title=element_text(size=20), 
        legend.position="right", 
        legend.title=element_blank());plot_species_site_acr

ggsave(plot_species_site_acr, filename="Figures/ITS2/acr-summary.jpeg", width=6, height=6)
ggsave(plot_species_site_acr, filename="Figures/ITS2/acr-summary.pdf", width=6, height=6)
...
plot_species_site_poc_haplotypes<-ggplot(plot_species_site_df_poc, aes(x = haplotype, y = Abundance)) +
  ...
  theme(text=element_text(size=20), 
        ...
        axis.title=element_text(size=20), 
        legend.position="right", 
        legend.title=element_blank());plot_species_site_poc_haplotypes

ggsave(plot_species_site_poc_haplotypes, filename="Figures/ITS2/poc-summary.jpeg", width=10, height=6)
ggsave(plot_species_site_poc_haplotypes, filename="Figures/ITS2/poc-summary.pdf", width=10, height=6)
```
...
plot_species_site_por_haplotype<-ggplot(plot_species_site_df_por, aes(x = haplotype, y = Abundance)) + 
  ...
  theme(text=element_text(size=20), 
        ...
        axis.title=element_text(size=20), 
        legend.position="right", 
        legend.title=element_blank());plot_species_site_por_haplotype

ggsave(plot_species_site_por_haplotype, filename="Figures/ITS2/por-summary.jpeg", width=13, height=6)
ggsave(plot_species_site_por_haplotype, filename="Figures/ITS2/por-summary.pdf", width=13, height=6)
```
...
Assemble haplotype plots. 
```{r}
ind_plot1a<-ind_plot1+theme(legend.position="none")+theme(text=element_text(size=20), strip.text=element_text(size=20), axis.text=element_text(size=20), axis.title=element_text(size=20))
ggsave(ind_plot1a, filename="Figures/ITS2/acr-panel.jpeg", width=12, height=6)
ggsave(ind_plot1a, filename="Figures/ITS2/acr-panel.pdf", width=12, height=6)

ind_plot2a<-ind_plot2+theme(legend.position="none")+theme(text=element_text(size=20), strip.text=element_text(size=20), axis.text=element_text(size=20), axis.title=element_text(size=20))
ggsave(ind_plot2a, filename="Figures/ITS2/poc-panel.jpeg", width=12, height=6)
ggsave(ind_plot2a, filename="Figures/ITS2/poc-panel.pdf", width=12, height=6)

ind_plot3a<-ind_plot3+theme(legend.position="none")+theme(text=element_text(size=20), strip.text=element_text(size=20), axis.text=element_text(size=20), axis.title=element_text(size=20))
ggsave(ind_plot3a, filename="Figures/ITS2/por_panel.jpeg", width=12, height=6)
ggsave(ind_plot3a, filename="Figures/ITS2/por_panel.pdf", width=12, height=6)
...
output_matrix%>%
  group_by(species, haplotype, timepoint)%>%
  summarise(n=length(unique(colony_id_corr)))
```

# Plot ITS2 profiles for E5 timeseries molecular project 
...
Plot colony ITS2 profiles. 

Acropora
```{r}
molecular_plot1<-output_matrix%>%
 filter(colony_id_corr %in% molecular)%>% 
  ...
  #facet_nested(~ species, scales = "free_x", space = "free_x") +
  labs(x="", fill="ITS2 Profile", y="Relative Abundance")+theme(text=element_text(size=20), strip.text=element_text(size=20), axis.text=element_text(size=20), axis.title=element_text(size=20), legend.position="right");molecular_plot1

ggsave(molecular_plot1, filename="Figures/ITS2/molecular_timeseries_its2_acr.png", width=8, height=6)
```

Pocillopora
```{r}
molecular_plot2<- output_matrix%>%
  filter(colony_id_corr %in% molecular)%>% 
  filter(species=="Pocillopora")%>%
  group_by(colony_id, species, haplotype)%>%
  summarise_if(is.numeric, mean, na.rm = TRUE, .groups = "drop")%>%
  ...
  #facet_nested(~ species, scales = "free_x", space = "free_x") +
  labs(x="", fill="ITS2 Profile", y="Relative Abundance")+theme(text=element_text(size=20), strip.text=element_text(size=20), axis.text=element_text(size=20), axis.title=element_text(size=20), legend.position="right");molecular_plot2

ggsave(molecular_plot2, filename="Figures/ITS2/molecular_timeseries_its2_poc.png", width=12, height=6)
```

Porites 
```{r}
molecular_plot3<- output_matrix%>%
  filter(colony_id_corr %in% molecular)%>% 
  filter(species=="Porites")%>%
  group_by(colony_id, species, haplotype)%>%
  summarise_if(is.numeric, mean, na.rm = TRUE, .groups = "drop")%>%
  ...
  #facet_nested(~ species, scales = "free_x", space = "free_x") +
  labs(x="", fill="ITS2 Profile", y="Relative Abundance")+theme(text=element_text(size=20), strip.text=element_text(size=20), axis.text=element_text(size=20), axis.title=element_text(size=20), legend.position="right");molecular_plot3

ggsave(molecular_plot3, filename="Figures/ITS2/molecular_timeseries_its2_por.png", width=10, height=6)
```
