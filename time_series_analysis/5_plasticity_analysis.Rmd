...
dist_all_plot2<-dist_all_mean%>%
  ...
  summarise(mean=mean(value), sd=sd(value), N=length(value), se=sd/sqrt(N))%>% 
    ...
    xlab("Site") + 
    ylab(expression(bold("Colony-Specific Euclidean Distance")))+
    geom_text(x=2, y=0.7, size=4, label="p(Site)<0.001", color="black")+
    geom_text(x=2, y=0.4, size=4, label="p(Species)<0.001", color="black")+
    geom_text(x=2, y=0.1, size=4, label="p(Site x Species)=0.040", color="black")+
    theme_classic() + 
    ylim(0,7)+
...
dist_holo_plot2<-dist_holo_mean%>%
  ...
  summarise(mean=mean(value), sd=sd(value), N=length(value), se=sd/sqrt(N))%>% 
    ...
    xlab("Site") + 
    ylab(expression(bold("Colony-Specific Euclidean Distance")))+
    geom_text(x=2, y=0.7, size=4, label="p(Site)=0.866", color="darkgray")+
    geom_text(x=2, y=0.4, size=4, label="p(Species)<0.001", color="black")+
    geom_text(x=2, y=0.1, size=4, label="p(Site x Species)=0.027", color="black")+
    theme_classic() + 
    ylim(0,7)+
    ggtitle("Holobiont Responses")+
    theme(
      legend.position="right",
      legend.title=element_text(face="bold", size=14),
      legend.text=element_text(size=14, face="italic"),
...
dist_sym_plot2<-dist_sym_mean%>%
  ...
  summarise(mean=mean(value), sd=sd(value), N=length(value), se=sd/sqrt(N))%>% 
    ...
    ylab(expression(bold("Colony-Specific Euclidean Distance")))+
    geom_text(x=2, y=0.7, size=4, label="p(Site)=0.001", color="black")+
    geom_text(x=2, y=0.4, size=4, label="p(Species)=0.004", color="black")+
    geom_text(x=2, y=0.1, size=4, label="p(Site x Species)=0.766", color="darkgray")+
    theme_classic() + 
    ylim(0,7)+
    ggtitle("Symbiont Responses")+
    theme(
      legend.position="right",
      legend.title=element_text(face="bold", size=14),
      legend.text=element_text(size=14, face="italic"),
...
panel<-plot_grid(dist_all_plot3, dist_holo_plot3, dist_sym_plot2, nrow=1, ncol=3, rel_widths = c(1,1,1.3), labels=c("A", "B", "C"), label_size=20, label_y=0.96)

ggsave(panel, file="Figures/Plasticity/Plasticity_Figure.png", width=18, height=5)
```
