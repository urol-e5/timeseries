...
```{r}
names(acr_data_temp)
acr.comm.rda.host<-dbrda(acr_data_temp[, c(6,9,11,14,17)] ~ mean + maximum + minimum + SD, 
                       data=acr_data_temp)
...
acr.comm.rda.sym<-dbrda(acr_data_temp[, c(7,8,10,12,13,15,16,18,19)] ~ mean + maximum + minimum + SD, 
                       data=acr_data_temp)
...
```{r}
names(por_data_temp)
por.comm.rda.host<-dbrda(por_data_temp[, c(6,9,11,14,17)] ~ mean + maximum + minimum + SD, 
                       data=por_data_temp)

##Check model significance
anova(por.comm.rda.host)
# Model: dbrda(formula = por_data_temp[, c(6, 9, 11, 14, 17)] ~ mean + maximum + minimum + SD + range, data = por_data_temp)
#           Df SumOfSqs      F Pr(>F)    
# Model      4   8.6155 13.652  0.001 ***
# Residual 134  21.1418 

plot(por.comm.rda.host)

##Check variance explained by model
(summary(por.comm.rda.host)$constr.chi/summary(por.comm.rda.host)$tot.chi)*100
#14.17% of the variance in host physiology is constrained by temperature metrics 
...
#dbRDA2 explains 12.06% of constrained variance 

##Check variance explained by each Host physiological metric
anova(por.comm.rda.host, by="terms")
# Model: dbrda(formula = por_data_temp[, c(6, 9, 11, 14, 17)] ~ mean + maximum + minimum + SD, data = por_data_temp)
#           Df SumOfSqs       F Pr(>F)    
# mean       1   6.1633 39.0641  0.001 ***
# maximum    1   1.8524 11.7409  0.001 ***
# minimum    1   0.3402  2.1562  0.110    
# SD         1   0.2596  1.6451  0.174    
# Residual 134  21.1418   
...
```{r}
names(por_data_temp)
por.comm.rda.sym<-dbrda(por_data_temp[, c(7,8,10,12,13,15,16,18,19)] ~ mean + maximum + minimum + SD, 
                       data=por_data_temp)

##Check model significance
anova(por.comm.rda.sym)
# Model: dbrda(formula = por_data_temp[, c(7, 8, 10, 12, 13, 15, 16, 18, 19)] ~ mean + maximum + minimum + SD, data = por_data_temp)
#           Df Variance      F Pr(>F)    
# Model      4  0.20855 9.7627  0.001 ***
# Residual 134  0.71561 

plot(por.comm.rda.sym)

##Check variance explained by model
(summary(por.comm.rda.sym)$constr.chi/summary(por.comm.rda.sym)$tot.chi)*100
#22.56% of the variance in symbiont physiology is constrained by temperature

RsquareAdj(por.comm.rda.sym)$adj.r.squared*100
#Adjusted: The model explains 20.25% of the variation in Symbiont physiology

summary(por.comm.rda.sym)$cont
#dbRDA1 explains 16.66% of total variance 
#dbRDA2 explains 4.93% of total variance 

summary(por.comm.rda.sym)$concont
#dbRDA1 explains 73.82% of constrained variance 
#dbRDA2 explains 21.82% of constrained variance 

##Check variance explained by each Symbiont physiological metric
anova(por.comm.rda.sym, by="terms")
# Model: dbrda(formula = por_data_temp[, c(7, 8, 10, 12, 13, 15, 16, 18, 19)] ~ mean + maximum + minimum + SD, data = por_data_temp)
#           Df Variance       F Pr(>F)    
# mean       1  0.05962 11.1649  0.001 ***
# maximum    1  0.07372 13.8039  0.001 ***
# minimum    1  0.03160  5.9167  0.001 ***
# SD         1  0.04360  8.1652  0.001 ***
# Residual 134  0.71561   
...
```{r}
names(poc_data_temp)
poc.comm.rda.host<-dbrda(poc_data_temp[, c(6,9,11,14,17)] ~ mean + maximum + minimum + SD, 
                       data=poc_data_temp)

##Check model significance
anova(poc.comm.rda.host)
# Model: dbrda(formula = poc_data_temp[, c(6, 9, 11, 14, 17)] ~ mean + maximum + minimum + SD, data = poc_data_temp)
#           Df SumOfSqs      F Pr(>F)    
# Model      4   2.4136 11.709  0.001 ***
# Residual 128   6.5962  
...
#26.79% of the variance in host physiology is constrained by temperature metrics 

RsquareAdj(poc.comm.rda.host)$adj.r.squared*100
#Adjusted: The model explains 24.50% of the variation in host physiology 

summary(poc.comm.rda.host)$cont
#dbRDA1 explains 23.28% of total variance 
#dbRDA2 explains 3.05% of total variance 

summary(poc.comm.rda.host)$concont
#dbRDA1 explains 86.92% of constrained variance 
#dbRDA2 explains 11.39% of constrained variance 
...
# Model: dbrda(formula = poc_data_temp[, c(6, 9, 11, 14, 17)] ~ mean + maximum + minimum + SD, data = poc_data_temp)
#           Df SumOfSqs       F Pr(>F)    
# mean       1   1.9632 38.0963  0.001 ***
# maximum    1   0.0539  1.0450  0.325    
# minimum    1   0.0483  0.9378  0.396    
# SD         1   0.3482  6.7563  0.002 ** 
# Residual 128   6.5962  
...
```{r}
names(poc_data_temp)
poc.comm.rda.sym<-dbrda(poc_data_temp[, c(7,8,10,12,13,15,16,18,19)] ~ mean + maximum + minimum + SD, 
                       data=poc_data_temp)
