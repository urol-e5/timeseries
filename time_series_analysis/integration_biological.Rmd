...
Correct all colony ID's and remove any colonies that are not collected in this data set (e.g., other projects, blanks).  

```{r}
#Change the coral id metadata from wide to long format
test<-tags%>%
  select(Original_Tag, Tag_ID_Jan, Tag_ID_Mar, Tag_ID_Nov, Tag_ID_Nov)%>%
  gather(Tag_ID_Jan:Tag_ID_Nov, timepoint_tag)
...
Turn id metadata into long format and then have it do an if else matching by "tag" and "timepoint" and populating by "original tag". 
...
Control for colony id, site name, and timepoint names. CHANGE TO IF ELSE FUNCTIONS TO AVOID SOME ISSUES 
```{r}
...
THIS PART IS MESSED UP, CHECK 
```{r}
#change colony id that was changed in the middle of the series
test<-master%>%
  mutate(colony_id=as.character(colony_id))%>%
  mutate(colony_id=if_else(colony_id=="POR-206", "POR-251", colony_id))
...
holobiont_responses<-c("cre.umol.mgafdw", "Host_AFDW.mg.cm2", "prot_mg.mgafdw", "Rd")
```
