...
```{r}
chl_figure<-plot_grid(chlaplot_prot, chlaplot_afdw, chlaplot_cm2, chlc2plot_prot, chlc2plot_afdw, chlc2plot_cm2, labels = c("", "", ""), ncol=3, nrow=2, rel_widths = c(.8, .8, 1, .8, .8, 1), label_size = 20, label_x = 0.1)

ggsave(filename="Figures/Chlorophyll_Figure.pdf", plot=chl_figure, dpi=300, width=24, height=10, units="in")
```
...
```{r}
master%>%
  select(cre.umol.mgafdw, prot_mg.mgafdw, Host_AFDW.mg.cm2, Sym_AFDW.mg.cm2, Ratio_AFDW.mg.cm2, chlc2.ug.mgAFDW, chla.ug.mgAFDW, cells.mgAFDW)%>%
...
```{r}
p<-master%>%
  select(species, cre.umol.mgafdw, prot_mg.mgafdw, Host_AFDW.mg.cm2, Sym_AFDW.mg.cm2, Ratio_AFDW.mg.cm2, chlc2.ug.mgAFDW, chla.ug.mgAFDW, cells.mgAFDW)%>%
  filter(Host_AFDW.mg.cm2>0)%>%
  filter(Sym_AFDW.mg.cm2>0)%>%
  filter(Ratio_AFDW.mg.cm2>0)%>%
  ggpairs(., columns=2:9, ggplot2::aes(colour=species)) #try to bold significant stats
