...
acr.comm.rda.host<-dbrda(vegdist(acr_data_community[, c(23:62)], "bray") ~ Antiox_Capacity + Host_Biomass + Host_Protein + Rd + Calc, 
...
```{r}
names(acr_data_community)
acr.comm.rda.sym<-dbrda(vegdist(acr_data_community[, c(23:62)], "bray") ~ Symbiont_Density + Symbiont_Biomass + Total_Chl + Total_Chl_cell + Am + AQY + Ik + Ic + S_H_Biomass_Ratio, data=acr_data_community, dist="bray")
...
por.comm.rda.host<-dbrda(vegdist(por_data_community[, c(23:62)], "bray") ~ Antiox_Capacity + Host_Biomass + Host_Protein + Rd + Calc, 
...
```{r}
names(por_data_community)
por.comm.rda.sym<-dbrda(vegdist(por_data_community[, c(23:62)], "bray") ~ Symbiont_Density + Symbiont_Biomass + Total_Chl + Total_Chl_cell + Am + AQY + Ik + Ic + S_H_Biomass_Ratio, data=por_data_community, dist="bray")

##Check model significance
anova(por.comm.rda.sym)
#           Df SumOfSqs      F Pr(>F)    
#Model      8   10.253 4.3733  0.001 ***
#Residual 126   36.924  
# The model is significant
...
#22.58% of the variance in Symbiont community composition is constrained by Host physiological metrics 
...
poc.comm.rda.host<-dbrda(vegdist(poc_data_community[, c(23:62)], "bray") ~ Antiox_Capacity + Host_Biomass + Host_Protein + Rd + Calc, 
...
```{r}
names(poc_data_community)
poc.comm.rda.sym<-dbrda(vegdist(poc_data_community[, c(23:62)], "bray") ~ Symbiont_Density + Symbiont_Biomass + Total_Chl + Total_Chl_cell + Am + AQY + Ik + Ic + S_H_Biomass_Ratio, data=poc_data_community, dist="bray")
...
names(acr_data_community)
acr.comm.rda.sym.scores<-merge(acr_data_community[,c(1:22, 63:64)], acr.comm.rda.sym.scores)
...
names(por_data_community)
por.comm.rda.sym.scores<-merge(por_data_community[,c(1:22, 63:64)], por.comm.rda.sym.scores)
...
names(poc_data_community)
poc.comm.rda.sym.scores<-merge(poc_data_community[,c(1:22, 63:64)], poc.comm.rda.sym.scores)
